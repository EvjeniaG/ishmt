...
export function InspectionContractsSection({ contracts }: { contracts: Contract[] }) {
  ...
  if (contracts.length === 0) {
    return (
      <PortalEmptyState>
        Nuk ka kontratë inspektimi. Caktoni OMI-n pas mirëmbajtjes aktive.
      </PortalEmptyState>
    );
  }
        ...
        <PortalTableWrap>
          ...
          <tbody>
            {filtered.map((contract) => (
              <tr key={contract.id} className={cn(contract === activeContract && "bg-gov-primary/[0.04]")}>
